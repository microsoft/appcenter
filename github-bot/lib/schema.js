...
const fields = {
  featureFirstComment: Joi.alternatives().try(Joi.string(), Joi.any().only(false))
    .error(() => '"featureFirstComment" must be a string or false')
    .description('Comment to post when marking as stale. Set to `false` to disable'),

  bugFirstComment: Joi.alternatives().try(Joi.string(), Joi.any().only(false))
    .error(() => '"bugFirstComment" must be a string or false')
    .description('Comment to post when marking as stale. Set to `false` to disable'),
...
const schema = Joi.object().keys({
  featureFirstComment: fields.markComment.default(
    'Welcome!'
  ),
  bugFirstComment: fields.markComment.default(
    'Sorry!'
  ),
